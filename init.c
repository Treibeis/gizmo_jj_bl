...
void init(void)
{
    ...
    All.TimeNextOnTheFlyFoF = get_next_FoF_time(HALODYNAMICALTIME);    
    /*
    double growth_timescale, redshift, da;
    redshift         = 1.0 / All.Time - 1.0;
    growth_timescale = HALODYNAMICALTIME * 2.5 / pow(1.0 + redshift,1.5);
    // large # to convert H(z) from km/s/Mpc --> 1/Gyr
    da = All.HubbleParam * 100. * All.Time * growth_timescale * 0.001022914943273906 *
      sqrt(All.OmegaLambda + All.Omega0 * pow(1. + redshift,3));

    All.TimeNextOnTheFlyFoF = All.Time + da;
    */
    if(All.TimeNextOnTheFlyFoF < ( 1./(1.+FOF_STARTING_REDSHIFT)) )
      All.TimeNextOnTheFlyFoF = ( 1./(1.+FOF_STARTING_REDSHIFT) );

    if(ThisTask == 0){
      printf("Setting first FOF for vwinds to a=%g (z=%0.2f)\n",
	     All.TimeNextOnTheFlyFoF, 1./All.TimeNextOnTheFlyFoF - 1.);
    }
#else
...
#ifdef FOFRAD
#ifdef FOF_SCALEDEPENDENT
    All.TimeNextOnTheFlyFoFRAD = get_next_FoF_time(0.01);
    if(All.TimeNextOnTheFlyFoFRAD < ( 1./(1.+FOF_STARTING_REDSHIFT)) )
      All.TimeNextOnTheFlyFoFRAD = ( 1./(1.+FOF_STARTING_REDSHIFT) );
    
    if(ThisTask == 0)
      printf("Setting first FOFRAD for MDwinds to a=%g (z=%0.2f)\n",
	     All.TimeNextOnTheFlyFoFRAD,1./All.TimeNextOnTheFlyFoFRAD-1.);
#else
    All.TimeNextOnTheFlyFoFRAD = All.TimeBegin;
#endif //fof_scaledependent
#endif //fofrad

#endif //ifdef blackholes || variablevelocity
    
#ifdef GALSF_JEANS_MIN_T
  double T0       = 1.e4; // Kelvin
  double yhelium  = (1. - HYDROGEN_MASSFRAC) / (4. * HYDROGEN_MASSFRAC);
  double mu       = (1. + 4. * yhelium) / (1. + yhelium + 0.);
  double gas_mass = 0.0;
  for(i = 0; i<NumPart; i++){
    if(P[i].Type == 0){
      gas_mass = P[i].Mass * All.UnitMass_in_g / All.HubbleParam;
      break;
    }
  }

  //calculate length for 1 Jeans mass in protons/cm^3 (no little h!)
  All.MinRhoThresh = 3. / (4. * M_PI * mu * PROTONMASS) * 
    pow((5.*BOLTZMANN*T0)/(GRAVITY * mu * PROTONMASS),3.) *
    (1./(All.DesNumNgb * gas_mass)) * (1./(All.DesNumNgb * gas_mass));

  //experimental to allow for resolving of multiple Jeans lengths
  All.MinRhoThresh /= (GALSF_JEANS_MIN_T*GALSF_JEANS_MIN_T*GALSF_JEANS_MIN_T);

  if(ThisTask==0){
    printf("JMT: [protons/cm^3] MinRhoThresh=%g\tPhysDensThresh=%g\n",All.MinRhoThresh,
	   All.HubbleParam * All.HubbleParam * 
	   All.PhysDensThresh/(PROTONMASS / HYDROGEN_MASSFRAC / All.UnitDensity_in_cgs));
  }

  //convert to g/cm^3
  All.MinRhoThresh *= PROTONMASS / HYDROGEN_MASSFRAC;
  //convert to gadget units (with little h!)
  All.MinRhoThresh /= All.UnitDensity_in_cgs;
  All.MinRhoThresh /= All.HubbleParam * All.HubbleParam;

  if(ThisTask==0){
    printf("JMT: [system units] MinRhoThresh=%g\tPhysDensThresh=%g\n",
	   All.MinRhoThresh,All.PhysDensThresh);
  }

  //take the maximum threshold
  if(All.PhysDensThresh > All.MinRhoThresh)
    All.MinRhoThresh = All.PhysDensThresh;
  if(ThisTask==0){
    printf("JMT: Setting MinRhoThresh=%g\n",All.MinRhoThresh);
  }
#endif
    ...
    for(i = 0; i < NumPart; i++)	/*  start-up initialization */
    {
        ...
        if(RestartFlag == 0)
        {
            if( P[i].Type == 0 ) All.OrigGasMass = P[i].Mass;
            P[i].StellarAge = 0;
#ifdef POPII_P2L
            P[i].Star_Type = 0;
#endif
	...
	All.SolarAbundances[0]=0.0134;      // Asplund et al. 2009
#else
...
#if defined(COOL_METAL_LINES_BY_SPECIES) || defined(GALSF_INSTANTANEOUS_METALS)
...
#endif
#ifdef POPII_P2L
        SphP[i].P2_heating_time = 0;
        SphP[i].P3_heating_time = 0;
        SphP[i].P2L_flag = 0;
 ...
 /* fractional abundance relative to nH or nHe */
        ...
        if(RestartFlag == 0)
        {
            ...
            SphP[i].Primordial_Chem[13]   = 0.0;                   // J_LW
        }
#endif
