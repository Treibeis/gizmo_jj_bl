...
#ifdef SINKS
    char buf[200];
    sprintf(buf, "%ssink", All.OutputDir);
    mkdir(buf, 02755);
#endif
...
void calculate_non_standard_physics(void)
{
	...
	/*
	double redshift, growth_timescale, da;
	redshift         = 1.0 / All.Time - 1.0;
	growth_timescale = HALODYNAMICALTIME * 2.5 / pow(1. + redshift,1.5);
	...
	*/
	if(ThisTask == 0)
	  printf("Doing FOF for winds at: a=%g (z=%0.2f) Nstep=%d, NextFOF=%g (z=%0.2f)\n",All.Time,
		 1./All.Time-1.,All.NumCurrentTiStep,All.TimeNextOnTheFlyFoF,1./All.TimeNextOnTheFlyFoF-1.); 
#else
        if(All.ComovingIntegrationOn)
            All.TimeNextOnTheFlyFoF *= All.TimeBetOnTheFlyFoF;
        else
            All.TimeNextOnTheFlyFoF += All.TimeBetOnTheFlyFoF;
#endif // ifdef FOF_SCALEDEPENDENT
    }
#endif // ifdef FOF

#ifdef FOFRAD
    if(All.Time >= All.TimeNextOnTheFlyFoFRAD){      
#ifdef FOF_SCALEDEPENDENT
      All.TimeNextOnTheFlyFoFRAD = get_next_FoF_time(0.01);
      if(ThisTask == 0)
	printf("Doing FOFRAD for MDwinds at: a=%g (z=%0.2f) Nstep=%d, NextFOF=%g (z=%0.2f)\n",All.Time,
	       1./All.Time-1.,All.NumCurrentTiStep,All.TimeNextOnTheFlyFoFRAD,1./All.TimeNextOnTheFlyFoFRAD-1.);
#else
    if(All.ComovingIntegrationOn)
      All.TimeNextOnTheFlyFoFRAD *= All.TimeBetOnTheFlyFoF;
    else
      All.TimeNextOnTheFlyFoFRAD += All.TimeBetOnTheFlyFoF;
...
#ifdef JJ_profile
    All.CPU_Sum[CPU_P3L], (All.CPU_Sum[CPU_P3L]) / All.CPU_Sum[CPU_ALL] * 100,
    All.CPU_Sum[CPU_P2L], (All.CPU_Sum[CPU_P2L]) / All.CPU_Sum[CPU_ALL] * 100,
    All.CPU_Sum[CPU_SFR], (All.CPU_Sum[CPU_SFR]) / All.CPU_Sum[CPU_ALL] * 100,
    All.CPU_Sum[CPU_COOL], (All.CPU_Sum[CPU_COOL]) / All.CPU_Sum[CPU_ALL] * 100,
#endif  
