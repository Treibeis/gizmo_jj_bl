...
void do_sinks(void)
{
  ...
  FILE *Fdsink_list;
  char buf[200];
  sprintf(buf, "%ssink/sink.%d.txt", All.OutputDir, ThisTask);
  if(!(Fdsink_list = fopen(buf, "a"))){
  printf("error in opening file '%s'\n",buf);
  endrun(1);
  }

  for(i = NumSinks = SinkMass = 0; i < NumPart; i++){
    if(P[i].Type == 3){
      NumSinks++;
      SinkMass += P[i].Mass/Msun_to_Internal;
      /*fprintf(Fdsink_list,"%g %g %g\t%g %g %g\t%g %g %g\t %lld\n",
              All.Time,P[i].Time,P[i].Mass/Msun_to_Internal,
              P[i].Pos[0], P[i].Pos[1], P[i].Pos[2], 
              P[i].Vel[0], P[i].Vel[1], P[i].Vel[2], P[i].sink_ID);
      fflush(Fdsink_list);*/
    }
  }

  //if(NumSinks>0)
  //  fprintf(Fdsink_list,"%g\n",All.Time);

  MPI_Allreduce(&NumSinks, &All.TotNumSinks, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&SinkMass, &All.TotSinkMass, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  if(ThisTask == 0) {fprintf(Fdsinkfr, "%g %d %g\n", All.Time, All.TotNumSinks, All.TotSinkMass);
  fflush(Fdsinkfr);}
  ...
  if(All.TotNumSinks > 0)
    ...
    //if(ThisTask == global.task){
      //if(!(All.sink_all = (struct sink_list *) malloc(All.TotNumSinks * sizeof(struct sink_list))))
      ...
      for(i = 0, index = prev_list[ThisTask]; i < NumSinks; i++)
	{
      ...
      sink_send[index].time = P[NumPart - NumSinks + i].Time;
      sink_send[index].ID = P[NumPart - NumSinks + i].sink_ID;

      /*cs = sink_send[index];
      fprintf(Fdsink_list,"%d %g %g\t%g %g %g\t%g %g %g\n",
              cs.index,cs.time,cs.mass/Msun_to_Internal,
              cs.pos[0], cs.pos[1], cs.pos[2], cs.vel[0], cs.vel[1], cs.vel[2]);
      fflush(Fdsink_list);*/
      ...
      //MPI_Allgatherv(&sink_send[prev_list[ThisTask]], NumSinks * sizeof(struct sink_list), MPI_BYTE, All.sink_all,
		//     count_list_bytes, prev_list_bytes, MPI_BYTE, MPI_COMM_WORLD);
	...
	if(index > -1 && semimajor_axis > 0 && semimajor_axis < All.SinkHsml * hubble_param)
	  {
        ...
        if(sink[i].time > sink[index].time){
          sink[i].time = sink[index].time;
			sink[i].ID = sink[index].ID;}

	    if(ThisTask == sink[i].task)
	      {
		P[NumPart - NumSinks + sink[i].index].Mass = sink[i].mass;
        P[NumPart - NumSinks + sink[i].index].Time = sink[i].time;
        P[NumPart - NumSinks + sink[i].index].sink_ID = sink[i].ID;
  ...
  if(global.nh_max > All.SinkDensThresh)
      ...
      if(count_global)
	{
...
#endif
          sink_send[index].time = All.Time;
          sink_send[index].ID = All.TotNumSinks;
	  ...
	  MPI_Allgatherv(&sink_send[prev_list[ThisTask]], count * sizeof(struct sink_list), MPI_BYTE, sink,
			 count_list_bytes, prev_list_bytes, MPI_BYTE, MPI_COMM_WORLD);
	  //MPI_Allgatherv(&sink_send[prev_list[ThisTask]], count * sizeof(struct sink_list), MPI_BYTE, sink_all,
			 //count_list_bytes, prev_list_bytes, MPI_BYTE, MPI_COMM_WORLD);
	  ...
	  if(e_grav > (2 * e_therm + e_mag) && e_grav > e_kin + e_therm + e_mag && divv < 0)
	      ...
	      if(num_acc_glob)
		{
		  ...
		  if(ThisTask == global.task)
		    {
		      ...
		      P[NumPart - 1] = psave;
              P[NumPart - 1].Time = All.Time;
              P[NumPart - 1].sink_ID = All.TotNumSinks;
		  ...
		  if(ThisTask == 0)
		    {
		      printf("Created sink #%lld at %g, %g, %g with %d particles for a total of %g M_sun\n",
  ...
  if(ThisTask == 0)
    {
      ...
      fflush(stdout);
      /*sum_sinkm = 0.0;
      if(num_update_glob>0){
      char buf[200];
      sprintf(buf, "%ssink/sink.%d.txt", All.OutputDir, All.count_output);
      if(!(Fdsink_list = fopen(buf, "a"))){
      printf("error in opening file '%s'\n",buf);
      endrun(1);
      }
      ...
      for(i = 0; i < All.TotNumSinks; i++){
      cs = sink[i];
      sum_sinkm += cs.mass/Msun_to_Internal;
      if(num_update_glob>0){
      fprintf(Fdsink_list,"%d %g %g\t%g %g %g\t%g %g %g\n",
              cs.index,cs.time,cs.mass/Msun_to_Internal,
              cs.pos[0], cs.pos[1], cs.pos[2], cs.vel[0], cs.vel[1], cs.vel[2]);
