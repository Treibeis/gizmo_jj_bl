  ...
  if(RestartFlag == 0 || RestartFlag == 2 || RestartFlag == 3 || RestartFlag == 4 || RestartFlag == 5 || RestartFlag == 6)
  ...
  else
    {
      ...
      */
      ...
      All.TimeNextOnTheFlyFoF = All.Time;
#ifdef FOFRAD
      All.TimeNextOnTheFlyFoFRAD = All.Time;
#endif
#endif
...
#ifdef SINKS
  sprintf(buf, "%s%s", All.OutputDir, "sinkfr.txt");
  if(!(Fdsinkfr = fopen(buf, mode)))
    {
      printf("error in opening file '%s'\n", buf);
      endrun(1);
    }
...
#ifdef POPII_P2L
  sprintf(buf, "%s%s", All.OutputDir, "popII_sfr.txt");
  if(!(FdPopIIsfr = fopen(buf, mode)))
    {
      printf("error in opening file '%s'\n", buf);
      endrun(1);
    }
  ...
  if(ThisTask == 0)		/* read parameter file on process 0 */
    {
      ...
      /* Needed for All.EgySpecSN, which is needed by subgrid winds 
       * also needed by JMT to calculate electron abundance */
      ...
      /* FactorSN needed for subgrid winds, regardless of EQS */                
