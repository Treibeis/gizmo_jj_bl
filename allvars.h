...
#else
#ifdef MULTI_COMPONENT_METALS
#define NUM_METAL_SPECIES 3				// Metallicity[0] = total, Metallicity[1]=PopII, Metallicity[2]=PopIII
#else
...
#define HALODYNAMICALTIME 0.05
#define FOF_STARTING_REDSHIFT 30
#endif
...
extern int *Ngblist;		/*!< Buffer to hold indices of neighbours retrieved by the neighbour search
				   routines */
extern int *Ngblist_Ifront;			// for popIII I-front neighbour search
...
#ifdef SINKS
extern FILE *Fdsinkfr;
#endif

#ifdef POPIII
extern FILE *FdPopIIIsfr;
#endif

#ifdef POPII_P2L
extern FILE *FdPopIIsfr;
#endif
...
#ifdef SINKS
extern long long NumSinks;
extern double SinkMass;
#endif
...
extern struct global_data_all_processes
{
  ...
  double POPII_C_SFRD;
  double POPII_SUM;
  //int POPII_flag;  // indicates particle has undergone P2 SF
...
// #if defined(POPIII) && defined(MULTI_COMPONENT_METALS)
//   double PopIII_Z;
//   double POPII_Z;
...
#ifdef GALSF_SFR_KMT
  double UnitSurfaceDensity_in_Msun_per_pc2;
#endif

#if defined(GALSF_EFFECTIVE_EQS) || defined(GALSF_SUBGRID_WINDS) || defined(GALSF_INSTANTANEOUS_METALS)
  double FactorSN;
  double EgySpecSN;
  double EgySpecCold;
#endif

#if defined(GALSF_EFFECTIVE_EQS) || defined(GALSF_SUBGRID_WINDS) || defined(GALSF_JEANS_MIN_T)
...
#ifdef POPII_P2L
    MyFloat Star_Type;	/* 0-not a star, 2-Pop II, 3-Pop III*/
#endif
...
#if defined(DO_DENSITY_AROUND_STAR_PARTICLES) || defined(GALSF_SFR_KMT)
...
#endif
#ifdef POPII_P2L
    //MyFloat PE_heating_time;  // remaining time until allowed to cool after a P3 SF event
    MyFloat P2_heating_time;  // remaining time until allowed to cool after a P2 SF event
    MyFloat P3_heating_time;  // remaining time until allowed to cool after a P3 SF event
    MyFloat P2L_flag;  // indicates particle has undergone P2 SF 0-no, 1-yes
#endif
#ifdef GALSF_SUBGRID_WINDS
  MyFloat DelayTime;                /*!< remaining maximum decoupling time of wind particle */
  int N_windLaunches;               /*!< number of times launched in a wind */
...
#endif
#endif
#if defined(GALSF_SUBGRID_VARIABLEVELOCITY) || defined(HOTGAS_QUENCH)
  MyFloat HostHaloMass;             /*!< host halo mass estimator for wind launching velocity */
#endif
#ifdef HOTGAS_PRECIP
  MyFloat FreeFallTime;             /*!< free fall time = sqrt(2r/M(<r)) */
#endif
...
#if defined(PRIMORDIAL_COOLING) && defined(COOLING)
    MyFloat Primordial_Chem[14]; /* Primordial chem for Primordial cooling implemented by JDJ based on V. Bromm 2002 */
    							 /* species HI, HII, H-, H2, H2+, e-, HeI, HeII, HeIII, DI, DII, HD, 
    							 final two are H2 shielding and J_LW */
#endif

// #if defined(PRIMORDIAL_COOLING) && defined(COOLING) && defined(LYMAN_WERNER)
//     MyFloat fshield;
// #endif
